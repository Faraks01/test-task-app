...
const CharactersList: FC<CharactersListProps> = () => {
  ...
  const {
    isLoading,
    data
  } = useGetCharactersList(page, debouncedSearchString);
  ...
  return (
    <Grid container spacing={4}>
      ...
      {!isLoading && data?.results?.map((character) => (
        <Grid
          key={character.id}
          xs={12}
          md={6}
        ...
        >
          <CharacterCard
            character={character}
            actions={(
              <NavigateToCharacterDetailsButton
                character={character}
              />
            )}
          />
      ...
      {isLoading && Array.from(Array(3).keys()).map((_, index) => (
        <Grid
